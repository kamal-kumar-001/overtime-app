    ...
    return (
        <SafeAreaView className="bg-primary h-full">
            <View className="flex-row justify-center items-center my-4 px-4">
                <TouchableOpacity className="absolute z-30 p-6 left-0 top-0" onPress={() => router.replace('/setting')}>
                    <Image
                        ...
                        <View className="mt-7">
                            ...
                            {shifts.map((shift, index) => (
                                <View key={index} className="mt-2 p-4 bg-black-200 rounded-lg">
                                    <TouchableOpacity className="absolute z-30 p-5 right-0 top-0" onPress={() => handleShiftDelete(shift.shiftName)} >
                                        <Image
                                            source={icons.trash}
                                            resizeMode="contain"
                                            tintColor='white'
                                    ...
                                    <Text className="text-white uppercase font-psemibold">Shift Name:{"    "} {shift.shiftName}</Text>
                                    <Text className="text-white uppercase font-psemibold">Check In: {"         "}{new Date(shift.checkIn).toLocaleTimeString()}</Text>
                                    <Text className="text-white uppercase font-psemibold">Check Out:{"     "} {new Date(shift.checkOut).toLocaleTimeString()}</Text>
                                </View>
                            ))}
                        </View>
                    ) : (
